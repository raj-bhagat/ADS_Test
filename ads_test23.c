...
static void _heapify_ (int32_t data[], uint32_t len, uint32_t parent,int32_t status[], int32_t ac_no[]) {
	...
	while (child <= len) {
		...
		_swap_(&status[parent], &status[child]);
		_swap_(&ac_no[parent], &ac_no[child]);
		parent = child;
		child = 2 * parent;
	}
}
...
Heap 	heap_new(int32_t data[], uint32_t len, int32_t status[], int32_t ac_no[]) {
	...
	for (idx = len/2; idx > 0; --idx) {
		_heapify_(data, len, idx,status,ac_no);
	}
	heap.size = len;
	memcpy(heap.data, data, (len+1) * sizeof(int32_t));
	memcpy(heap.status, status, (len+1) * sizeof(int32_t));
	memcpy(heap.ac_no, ac_no, (len+1) * sizeof(int32_t));
	_test_heap_(heap.data, heap.size);
...
Heap*	heap_sort(Heap *heap) {
	...
	for (idx = heap->size; idx > 1; idx--) {
		...
		_swap_(&heap->status[idx], &heap->status[1]);
		_swap_(&heap->ac_no[idx], &heap->ac_no[1]);
		_heapify_(heap->data, idx-1, 1, heap->status,heap->ac_no);
	}
...
/*Heap*	heap_insert(Heap *heap, int32_t key, int32_t status)
{
...
/*int32_t		heap_extract_min(Heap *heap)
{
	...
	--heap->size;
	_heapify_(heap->data, heap->size, 1, heap->status,heap->ac_no);
